...
test("projects: Update control does not overlap primary action at phone width", async (t) => {
  ...
  await app.page.waitForSelector(".gh-update-btn, .project-action-update");
  ...
  const metrics = await app.page.evaluate(() => {
    ...
    const update = row.querySelector(
      ".gh-update-btn, .project-action-update",
    ) as HTMLElement | null;
    ...
    const actions = row.querySelector(".col-actions") as HTMLElement | null;
    ...
    return {
      ...
      updateWidth: u?.width ?? null,
      actionsWidth: a?.width ?? null,
    };
  });
  ...
  assert.equal(
    ...
    false,
    "Update must not paint outside the GitHub column when hosted there",
  );
